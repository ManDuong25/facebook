...
public interface PostRepository extends JpaRepository<Post, Long> {
       ...
       Page<Post> findByUserIdAndContentContaining(@Param("userId") Long userId, @Param("searchTerm") String searchTerm,
                     Pageable pageable);
       ...
       @Query("SELECT p FROM Post p JOIN p.user u WHERE p.user.id = :userId AND " +
                     ...
                     "LOWER(CONCAT(u.firstName, ' ', u.lastName)) LIKE LOWER(CONCAT('%', :searchTerm, '%'))) " +
                     "AND p.deletedAt IS NULL")
       Page<Post> findByUserIdAndContentOrUserInfo(@Param("userId") Long userId, @Param("searchTerm") String searchTerm,
                     Pageable pageable);
