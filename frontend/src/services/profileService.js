...
export const uploadAvatar = async (userId, avatarFile) => {
  try {
    ...
    console.log('Preparing to upload avatar for user ID:', numericUserId);
    ...
    // In ra tất cả các key-value trong FormData để kiểm tra
    console.log('FormData entries:');
    for (let pair of formData.entries()) {
      console.log(pair[0], ':', pair[1]);
    }
    
    console.log('Sending POST request to http://localhost:8080/api/users/avatar');
    ...
    console.log('Avatar upload complete, server response:', responseData);
...
export const uploadCoverPhoto = async (userId, coverFile) => {
  try {
    ...
    console.log('Preparing to upload cover photo for user ID:', numericUserId);
    console.log('File details:', {
      name: coverFile.name,
      type: coverFile.type,
      size: coverFile.size
    ...
    console.log('Sending POST request to http://localhost:8080/api/users/cover');
    ...
    const responseData = await response.json();
    console.log('Cover photo upload complete, server response:', responseData);
